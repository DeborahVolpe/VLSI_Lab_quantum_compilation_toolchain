...
from compute_NMR_SwapErrorRates_nonRZvirtual import getRandomRotationsQasmLines, getCZQasmImplementation,\
    getCzUnitaryMatrix
...
def main():
    ...
    for swap in [[0, 1],[1, 2],[2, 3],[3, 4]]: # Generate all the possible swaps (only one swap per pair of 
        ...
        sumCzFidelity_nonRZvirtual = 0.0  
        for i in range(shots):
            ...
            qasmLines_nonRZvirtual = getRandomRotationsQasmLines(qubit1, qubit2, signExchInt, rx_phases)
            ...
            # Create the ideal Cz Unitary Matrix for the current Swap:
            idealUnitaryMatrix = getCzUnitaryMatrix(qubit1, qubit2, nQubits, signExchInt, rx_phases)
            ...
            # to-swap qubits) and a non-ideal Cz gate:

            # Add the Cz gate to the quantum circuit:
            f = open(qasm_file_name, "a")
            f.writelines( getCZQasmImplementation(qubit1, qubit2, signExchInt, rx_phases) )
            f.close()
            ...
            print( "Iteration {0}; CZ q[{1}], q[{2}]; quantum gate fidelity obtained in last simulation (non RZ virtual): {3}".format(
                                                                                      i, qubit1, qubit2, Czfidelity) )
            sumCzFidelity_nonRZvirtual += Czfidelity

        # Compute the average fidelity and error rate:
        avgCzFidelity_nonRZvirtual = sumCzFidelity_nonRZvirtual/shots
        avgCzErrRate_nonRZvirtual = 1 - avgCzFidelity_nonRZvirtual

        # Print result:
        f = open(reportFilePath, "a")
        f.write("CZ q[{0}], q[{1}]; average quantum gate error rate (non RZ virtual): {2}\n".format(qubit1, qubit2, avgCzErrRate_nonRZvirtual))
        f.close()
...
if __name__=="__main__":
    ...
    reportFilePath = "./results/QuantumDot_Cz_error_rates_nonRZvirtual.txt"
